...
tipam_prefix().create_table()
tipam_prefix_tag().create_table()

with db() as conn:
    conn.execute('DELETE from tipam_prefix')
    conn.execute('DELETE FROM tipam_prefix_tag')
...
def test_add_supernet():
    ...
    with db() as conn:
        sql = "SELECT * FROM tipam_prefix WHERE id=?"
...
def test_find_initial():
    ...
    prefix = IPNetwork('196.25.0.0/24')
    ip = prefix._ip
    assert result['a4'] == ip
    ip = int_128_to_32(ip)
    with db() as conn:
        sql = "SELECT id FROM tipam_prefix " \
...
def test_find_second():
    ...
    prefix = IPNetwork('196.25.1.0/24')
    ip = prefix._ip
    assert result['a4'] == ip
    ip = int_128_to_32(ip)
    with db() as conn:
...
def test_add_child_with_allocated_children():
    ...
    with db() as conn:
        ...
        new_parent = conn.execute("SELECT parent FROM tipam_prefix WHERE "
                                  "a4=3289972736 AND prefix_len=24").fetchone()
        new_parent = new_parent['parent']
        new_parent2 = conn.execute("SELECT parent FROM tipam_prefix WHERE "
...
def test_add_prefix_above_supernet():
    ...
    with db() as conn:
        sql = "SELECT * FROM tipam_prefix WHERE a4=?"
...
def test_allocate_from_empty_child_prefix():
    ...
    assert alloc_30['a4'] == IPNetwork('196.25.2.0/30')._ip
    with db() as conn:
        sql = "SELECT id,parent FROM tipam_prefix " \
...
def test_delete_prefix_with_assigned_children():
    ...
    with db() as conn:
        sql = "SELECT * FROM tipam_prefix WHERE a4=? AND " \
...
def test_delete_prefix_with_child():
    ...
    assert len(rows) > 1
    assert rows[0]['prefix_type']=='hidden'
...
def test_release_only_assignment_in_parent():
    ...
    with db() as conn:
        sql = "SELECT * FROM tipam_prefix WHERE a4>=? AND a4<=? " \
...
def test_speed():
    ...
    assert duration<10
    with db() as conn:
        count = conn.execute("SELECT count(id) AS n FROM tipam_prefix "
